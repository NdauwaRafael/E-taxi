...
$check = "SELECT * FROM `taxiowner` WHERE `email`='$email'";
...
}else
{
            if (!empty($fname) && !empty($lname) && !empty($email) && !empty($password) && !empty($phone) && !empty($gender)) {/*check whether fields are empty*/
                    ...
                    $query ="INSERT INTO `taxiowner`(`id`, `firstname`, `lastname`, `email`, `phone`, `gender`, `password`) VALUES (NULL,'$fname','$lname','$email','$phone','$gender','$password')";
                    /*run sql query*/
